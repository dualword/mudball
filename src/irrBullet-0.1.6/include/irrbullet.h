...
/*!
    @mainpage irrBullet 0.1.6 wrapper documentation
    ...
    the Irrlicht rendering engine. irrBullet is easy to use, has a similar programming
    style with Irrlicht's style, and is powerful. It also adds features that
    are game-specific to let the physics programmer focus on actually doing the fun part of physics,
    not worrying about how to integrate and optimize it.

    It does not simply bind Bullet with Irrlicht, it includes unique features to help with the
    integration and prototyping of physics simulation!

    If you have any questions or suggestions, please email them to me at fighterstw (at) hotmail.com.


    @section irrbulletexample irrBullet Example

    Using irrBullet with Irrlicht is very straight-forward and simple. This does not
    mean it is not powerful, however.

    Here's a simple integration:
    ...
    That's it! You now have a pretty basic irrBullet example with a static triangle mesh.
    ...
    This is for most features with GImpact included. Other libs must be linked against for some features,
    ...
    Next, add these directories to your Search Directories list:
        ...
        <b><irrBulletDirectory>

        <irrBulletDirectory> / include / bheaders / bullet</b>
